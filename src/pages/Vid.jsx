...
const Vid = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_BASE_URL}/videos/${id}`, {
      ...
      .then((data) => {
        ...
        fetch(
          `${process.env.REACT_APP_BASE_URL}/videos/categories/${category}`,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const follow = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/auth/profile/follow`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const unfollow = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/auth/profile/unfollow`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const checkIfFollwed = async (token, username) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/auth/profile/followed`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const checkIfVideoDisliked = async (token) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/videos/${id}/disliked`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const checkIfVideoLiked = async (token) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/videos/${id}/liked`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleLike = async (e) => {
    ...
    if (dislike) {
      await fetch(
        `${process.env.REACT_APP_BASE_URL}/videos/${id}/removeDislike`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-11/12 mt-6 flex flex-col gap-2">
        <VidPlayer video={vid} />
        <div className="text-2xl font-bold ml-4 mt-6">{vid.videoName}</div>
        <hr />
        <div className="flex flex-col gap-4">
          ...
          <div
            onClick={() => setDescExpand((desc) => !desc)}
        ...
        <div className="flex items-center gap-2">
          <div>
            <img
              className="w-[50px] h-[50px] rounded-[50%] border border-black"
          ...
          {followed ? (
            <div
              onClick={unfollow}
              className="border border-black p-2 rounded-md bg-green-600 cursor-pointer"
            >
              Followed
            </div>
          ) : (
            <div
              onClick={follow}
              className="border border-black rounded-md p-2 cursor-pointer hover:bg-red-200 hover:border-red-700 transition duration-150 ease-out hover:ease-in"
            >
              Follow
            </div>
          )}
        ...
        {showComment ? (
          <div>
            ...
            <button
              ...
              className="border border-black mt-2 rounded-md p-2 cursor-pointer hover:bg-green-200 hover:border-green-600 transition duration-150 ease-out hover:ease-in"
            >
              Collapse Comments
            </button>
          </div>
          ...
          <div className="border border-black flex items-center justify-center rounded-md p-4 mt-4">
            <button
              ...
              className="border border-black rounded-md p-2 cursor-pointer hover:bg-green-200 hover:border-green-600 transition duration-150 ease-out hover:ease-in"
            >
              Load Comments
            </button>
          </div>
